...
def getRandomChests(numChests):
    # Create a list of chests (two-item lists of x, y int coords)
    chests = []
    while len(chests) < numChests:
        newChest = [random.randint(0, 59), random.randint(0, 14)]
        if newChest not in chests: # Make sure a chest is not already there
...
def makeMove(board, chests, x, y):
    # Change the board with a sonar device character.
    # Remove chests from the chests list as they are found.
...
while True:
    ...
    while sonarDevices > 0:
        ...
        print('You have %s sonar device(s) left. %s treasure chest(s) remaining.' % (sonarDevices, len(chests)))

        x, y = enterPlayerMove(previousMoves)
        previousMoves.append([x, y]) # Track all moves so that sonar devices can be updated.

        moveResult = makeMove(board, chests, x, y)
        if moveResult == False:
            continue
        else:
